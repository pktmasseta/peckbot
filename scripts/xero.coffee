...
module.exports = (robot) ->
  ...
  robot.respond /xero (.+)$/, (res) ->
    ...
    if not user.xero_userid?
      res.send "You haven't been set up with xero yet. Try running `xero member add <your slack name>`. If that doesn't work, ask pkt-it. Once you've done that, you can try your last command again."
      return
    if command == 'help' or command == 'start'
      res.send "I can file PKT reimbursements for you. I'll collect information in a private message, but first go upload your receipt at the following link:\n\n#{getURL(res)}"
      return
    if command == 'cancel'
      handleCancel(robot, res, user)
